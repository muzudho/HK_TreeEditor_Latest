...
namespace TreeEditor
{
    public partial class UiMain : UserControl
    {
        ...
        public string GetRtfFileFull(string projectFolder, string nodeFile, bool isBackup)
        {
            StringBuilder sb = new StringBuilder();
        ...
        public void CreateSampleRtfFile2(string rel, out string contents)
        {
            contents = "";

            string abs = this.GetRtfFileFull(this.Contents.ProjectFolder, rel, false);
        ...
        public void CreateDefaultProject(string projectFolder, bool isCreateDammyData)
        {
            ...
            if (isCreateDammyData)
            {
                ...
                {
                    string contents;
                    this.CreateSampleRtfFile2( @"node\飲み物.rtf", out contents);
                }
                ...
                {
                    string contents;
                    this.CreateSampleRtfFile2( @"node\果物.rtf", out contents);
                }
                ...
                {
                    string contents;
                    this.CreateSampleRtfFile2( @"node\食べ物.rtf", out contents);
                }
                ...
                {
                    string contents;
                    this.CreateSampleRtfFile2( @"node\野菜.rtf", out contents);
                }
            }
            ...
            if (e.Button == MouseButtons.Left)
            {
                if (ht.Location == TreeViewHitTestLocations.Label)
                {
                    ...
                    if (isLoad)
                    {
                        ...
                        if (!File.Exists(this.GetRtfFileFull(this.Contents.ProjectFolder, rtfRel, false)))
                        {
                            // なければ作る。
                            string contents;
                            this.CreateSampleRtfFile2(rtfRel, out contents);
