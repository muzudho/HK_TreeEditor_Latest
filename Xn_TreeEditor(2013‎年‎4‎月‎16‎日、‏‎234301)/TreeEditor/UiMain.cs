...
namespace TreeEditor
{
    public partial class UiMain : UserControl
    {
        ...
        public string GetTreeCsvFile(string projectName, bool isBackup)
        {
            ...
            if (isBackup)
            {
                ...
                sb.Append(this.GetProjectFolder(projectName));
                sb.Append(@"\backup\");
                sb.Append(time);
        ...
        public string GetTextFile(string projectName, string nodeName, bool isBackup)
        {
            StringBuilder sb = new StringBuilder();
            if (isBackup)
            {
        ...
        public string GetResourceCsvFile(string projectName, string nodeName, bool isBackup)
        {
            StringBuilder sb = new StringBuilder();
            if (isBackup)
            {
        ...
        public string GetListiconFolder(string projectName, string nodeName)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append(this.GetProjectFolder(projectName));
        ...
        public void OpenProject(string projectName)
        {
            ...
            //━━━━━
            {
                string file = this.GetTreeCsvFile(this.Contents.ProjectName,false);
                if (File.Exists(file))
                {
                    try
                    {
            ...
            //━━━━━
            {
                if (this.ParentForm is Form1)//ビジュアルエディターでは、親フォームがForm1とは限らない。
                {
                    Actions.LoadPage(
                        ...
                        this.GetTextFile(this.Contents.ProjectName, this.uiTextside1.NodeNameTxt1.Text, false),//ファイルパス
                        this.GetResourceCsvFile(this.Contents.ProjectName, this.uiTextside1.NodeNameTxt1.Text, false)
                        );
                }
            }
            ...
            System.Console.WriteLine("OpenProject情報　プロジェクト名："+this.Contents.ProjectName);
            System.Console.WriteLine("OpenProject情報　プロジェクトのファイルパス："+this.GetProjectFolder(this.Contents.ProjectName));
        ...
        public void CreateDefaultProject(string projectName2)
        {
            ...
            //━━━━━
            {
                string filePath = this.GetTreeCsvFile(projectName2, false);
                if (!File.Exists(filePath))
                {
                    string[] lines = new string[]{
            ...
            //━━━━━
            {
                ...
                this.CreateSampleTextFile1(
                    this.GetTextFile(projectName2, "飲み物", false),
                    out contents
                    );
            }
            //テキスト
            {
                string file = this.GetTextFile(projectName2, "果物", false);
                if (!File.Exists(file))
                {
                    string[] lines = new string[]{
                        "--------------------------------------------------",
            ...
            {
                string contents;
                this.CreateSampleResourceCsvFile1(this.GetResourceCsvFile(projectName2, "食べ物", false), out contents);
            }
            {
                string filePath = this.GetTextFile(projectName2, "食べ物", false);
                if (!File.Exists(filePath))
                {
                    string[] lines = new string[]{
                        "--------------------------------------------------",
            ...
            {
                string filePath = this.GetTextFile(projectName2, "野菜", false);
                if (!File.Exists(filePath))
                {
                    string[] lines = new string[]{
                        "--------------------------------------------------",
            ...
            if (e.Button == MouseButtons.Left)
            {
                if (ht.Location == TreeViewHitTestLocations.Label)
                {
                    ...
                    if (isLoad)
                    {
                        ...
                        //テキストファイルの存在有無確認
                        //━━━━━
                        string textFile = this.GetTextFile(this.Contents.ProjectName, ht.Node.Text, false);
                        ...
                        //テキストファイルの存在有無確認
                        //━━━━━
                        string csvFile = this.GetResourceCsvFile(this.Contents.ProjectName, ht.Node.Text, false);
        ...
        //    if (!isChanged)
        //    {
        //        if (this.Contents.IsChangedTree)
        //        {
        //            isChanged = true;
        //        }
        //    }

        //    this.Contents.IsChangedProject = isChanged;
            ...
            if (null != this.TreeView1.SelectedNode && this.TreeView1.SelectedNode is TreeNode2)
            {
                ...
                if (isSuccessful)
                {
                    ...
                    if (this.TreeView1.SelectedNode.Text != dlg.NodeNameText)
                    {
                        ...
                        //移動元ファイル名
                        string srcTextFile = this.GetTextFile(this.Contents.ProjectName, this.TreeView1.SelectedNode.Text, false);
                        string srcCsvFile = this.GetResourceCsvFile(this.Contents.ProjectName, this.TreeView1.SelectedNode.Text, false);
                        //移動先ファイル名
                        string dstTextFile = this.GetTextFile(this.Contents.ProjectName, dlg.NodeNameText, false);
                        string dstCsvFile = this.GetResourceCsvFile(this.Contents.ProjectName, dlg.NodeNameText, false);
        ...
        private void toolStripButton22_Click(object sender, EventArgs e)
        {
            ...
            if (null != this.TreeView1.SelectedNode.Parent)
            {
                ...
                {
                    string contents;
                    this.CreateSampleTextFile1(this.GetTextFile(this.Contents.ProjectName, nodeName, false), out contents);
                }

                //CSV
                {
                    string contents;
                    this.CreateSampleResourceCsvFile1(this.GetResourceCsvFile(this.Contents.ProjectName, nodeName, false), out contents);
                }
