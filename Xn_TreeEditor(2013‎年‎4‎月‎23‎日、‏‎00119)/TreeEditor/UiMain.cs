...
namespace TreeEditor
{
    public partial class UiMain : UserControl
    {
        ...
        public void OpenProjectLaw(string projectName, string projectFolder)
        {
            ...
            //━━━━━
            {
                string absTree = this.GetTreeCsvFile(projectFolder, false);
                if (File.Exists(absTree))
                {
                    try
                    {
                        string text = File.ReadAllText(absTree, Encoding.GetEncoding("Shift_JIS"));
                        ...
                        {
                            System.Console.WriteLine("★ツリーCSV読込完了 [" + absTree + "]");
                            ...
                            table.ForEach_Datapart(delegate(Record_Humaninput recordH, ref bool isBreak2, Log_Reports log_Reports2)
                            {
                                ...
                                string rText = recordH.TextAt("TEXT");
                                ...
                                System.Console.WriteLine("NO=" + rNo + " EXPL=" + rExpl + " TREE=" + rTree + " ICON=" + rIcon + " NAME=" + rText + " FILE=" + rFile + " FORE_COLOR" + rForeColor + " BACK_COLOR" + rBackColor);

                                int iconN;
                                int.TryParse(rIcon, out iconN);

                                TreeNode2 tn = this.CreateTreeNode(null, rText, rFile, iconN, rForeColor, rBackColor);
                    ...
                    {
                        MessageBox.Show( this.ParentForm, "OpenProjectLaw　e=" + e.Message + "]", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
                {
                    ...
                    string s = "OpenProjectLaw　ツリーCSV[" + absTree + "]がない。\nprojectFolder[" + projectFolder + "]";
                    System.Console.WriteLine(s);
                    //essageBox.Show(this.ParentForm, s, "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            ...
            if (0 < this.TreeView1.Nodes.Count)
            {
                ...
                this.Contents.RtfRel = ((TreeNode2)this.TreeView1.Nodes[0]).RtfRel;
            }
            else
            {
                this.uiTextside1.NodeNameTxt1.Text = "";
                this.Contents.RtfRel = "";
            }
            //essageBox.Show("OpenProjectLaw　rtfRel[" + this.Contents.RtfRel + "]　ルート数=[" + this.TreeView1.Nodes.Count + "]");
            ...
            System.Console.WriteLine("OpenProject情報　プロジェクト名：" + projectName);
            System.Console.WriteLine("OpenProject情報　プロジェクトのファイルパス：" + projectFolder);
        ...
        public void CreateDefaultProjectLaw(string projectFolder, bool isCreateDammyData)
        {
                ...
                if (!File.Exists(file))
                {
                    ...
                    if (isCreateDammyData)
                    {
                        lines = new string[]{
                            ...
                            @",,1,0,食べ物,node\食べ物.rtf,",
                            @",,2,0,果物,node\果物.rtf,",
                            @",,2,1,野菜,node\野菜.rtf,",
                            @",,1,0,飲み物,node\飲み物.rtf,",
                            @"EOF,,,,,,",
                        };
                    }
                    else
            ...
            if (isCreateDammyData)
            {
                ...
                {
                    string contents;
                    Utility.CreateRtf(this, projectFolder, @"node\飲み物.rtf", out contents);
                    //essageBox.Show("新規作成：「飲み物.rtf」作成完了。\ncontents=[" + contents+"]");

                    Utility.CreateCsv(this, projectFolder, @"node\飲み物.txt", out contents);
                    Utility.CreateText(this, projectFolder, @"node\飲み物.txt", out contents);
                }
                ...
                {
                    string contents;
                    Utility.CreateRtf(this, projectFolder, @"node\果物.rtf", out contents);
                    Utility.CreateCsv(this, projectFolder, @"node\果物.txt", out contents);
                    Utility.CreateText(this, projectFolder, @"node\果物.txt", out contents);
                }
                ...
                {
                    string contents;
                    Utility.CreateRtf(this, projectFolder, @"node\食べ物.rtf", out contents);
                    Utility.CreateCsv(this, projectFolder, @"node\食べ物.txt", out contents);
                    Utility.CreateText(this, projectFolder, @"node\食べ物.txt", out contents);
                }
                ...
                {
                    string contents;
                    Utility.CreateRtf(this, projectFolder, @"node\野菜.rtf", out contents);
                    Utility.CreateCsv(this, projectFolder, @"node\野菜.txt", out contents);
                    Utility.CreateText(this, projectFolder, @"node\野菜.txt", out contents);
                }
            }
        ...
        /// 保存の確認
        /// </summary>
        /// <param name="isMsgboxUsed">メッセージボックスを使ったら真を返します。</param>
        /// <param name="loadNodeText">もしロードが必要になったとき、ノードのテキストです。</param>
        /// <param name="createRtfRel">もしRTFファイルの作成が必要になったとき、そのプロジェクトからの相対パスです。</param>
        public void ConfirmSave(out bool isMsgboxUsed, string loadNodeText, string createRtfRel, bool isSaveTreeToo)
        {
            //
            //　　　　　　　　　　　　ページを　　　　　　　　　　　ページの
            //　　　　　　　　　　　　変更していれば　　　　　　　　変更がなければ
            ...
            // ツリー構造の更新は無視します。ページの更新だけ判定します。
            if (
                (isSaveTreeToo&&this.Contents.IsChangedProject)
                ||
                (!isSaveTreeToo&&this.Contents.IsChangedPage)
                )
            {
                //（１）
                StringBuilder sb = new StringBuilder();
            ...
            if (isLoad)
            {
                ...
                //ノード名読込
                //━━━━━
                this.UiTextside1.NodeNameTxt1.Text = loadNodeText;


                //━━━━━
                //RTFファイルの存在有無確認
                //━━━━━
                string rtfRel = createRtfRel;
                if (!File.Exists(this.GetFileFull(this.Contents.ProjectFolder, rtfRel, false)))
                {
                    // なければ作る。
                    string contents;
                    Utility.CreateRtf(this, this.Contents.ProjectFolder, rtfRel, out contents);
                ...
                if (!File.Exists(this.GetFileFull(this.Contents.ProjectFolder, csvRel, false)))
                {
                    // なければ作る。
                    string contents;
                    Utility.CreateCsv(this, this.Contents.ProjectFolder, csvRel, out contents);
        ...
        private void treeView1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                {
                    bool isMsgboxUsed;
                    this.ConfirmSave(out isMsgboxUsed, ht.Node.Text, ((TreeNode2)ht.Node).RtfRel,
                        false//ツリー構造の変化は無視
                        );
