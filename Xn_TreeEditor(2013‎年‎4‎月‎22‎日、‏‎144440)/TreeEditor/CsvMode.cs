                    ...
                    case MouseButtons.Left:
                        {
                            ...
                            this.mouseDownLocation = new Point(e.Location.X, e.Location.Y);
                        }
                        break;
        ...
        public void Pic1_MouseMove(Form1 form1, object sender, MouseEventArgs e)
        {
            if (form1.UiMain1.UiTextside1.IsImageCsvMode)
            {
                if (null != this.DraggingPictBox)
                {
                    ...
                    //// テキストエリア内でのクリック点
                    //Point p2 = new Point(
                    //    this.DraggingPictBox.Location.X + e.Location.X,
                    //    this.DraggingPictBox.Location.X + e.Location.Y
                    //);

                    ////Point p1 = form1.UiMain1.UiTextside1.PointToClient(e.Location);

                    //// リッチテキストエリア内での、ピクチャーボックスの左上座標
                    //System.Console.WriteLine("Pic1_MouseMove　ヘッド（" + this.RichScrollbar.Head.X + "、" + this.RichScrollbar.Head.Y + "）");
                    //System.Console.WriteLine("Pic1_MouseMove　e.Location（" + e.Location.X + "、" + e.Location.Y + "）");
                    //System.Console.WriteLine("Pic1_MouseMove　テキストエリア内（" + p2.X + "、" + p2.Y + "）");

                    //// 画像の中心座標に変換
                    //this.DraggingPictBox.LocationInAll = new Point(
                    //    this.RichScrollbar.Head.X + p2.X - this.DraggingPictBox.Width/2,
                    //    this.RichScrollbar.Head.Y + p2.Y - this.DraggingPictBox.Height/2
                    //    );

                    Point p2 = form1.UiMain1.UiTextside1.RichTextBox1.PointToClient(Cursor.Position);
                    System.Console.WriteLine("Pic1_MouseMove　ヘッド（" + this.RichScrollbar.Head.X + "、" + this.RichScrollbar.Head.Y + "）");
                    System.Console.WriteLine("Pic1_MouseMove　Cursor（" + Cursor.Position.X + "、" + Cursor.Position.Y + "）");
                    System.Console.WriteLine("Pic1_MouseMove　Client（" + p2.X + "、" + p2.Y + "）");

                    // 画像の中心座標のまま指定
                    this.DraggingPictBox.LocationCInAll = new Point(
                        this.RichScrollbar.Head.X + p2.X,
                        this.RichScrollbar.Head.Y + p2.Y
                        );
